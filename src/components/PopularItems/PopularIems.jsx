      ...
      <div className="grid-container">
       
        {
          ...
          ( data?.popularProductsArray?.map((product, index) => (
              <PopularProduct key={index} name={product.name} price={product.price} id={product.id} image={product?.imageGalleryImages[Math.floor(Math.random() * 6)].imageURL} collectionString={product.collectionString} productObj={product} />
          ))) 
          : 
